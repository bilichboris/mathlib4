...
/-- A subspace is invariant under a `*`-subalgebra of bounded operators iff its orthogonal
projection belongs to the commutant of that subalgebra. -/
theorem starProjection_mem_centralizer_iff_invtSubmodule
    (S : StarSubalgebra ℂ (H →L[ℂ] H))
    (K : Submodule ℂ H) [K.HasOrthogonalProjection] :
    K.starProjection ∈ Set.centralizer (S : Set (H →L[ℂ] H)) ↔
      ∀ a ∈ S, K ∈ Module.End.invtSubmodule (a : H →ₗ[ℂ] H) := by
  constructor
  · intro hp a ha
    have hcomm' : Commute a K.starProjection := hp a ha
    have hcomm : Commute K.starProjection a := hcomm'.symm
    exact Submodule.invtSubmodule_of_commute_starProjection (U := K) (a := a) hcomm
  · intro hK
    have hKorth : ∀ {a : H →L[ℂ] H}, a ∈ S → Kᗮ ∈ Module.End.invtSubmodule (a : H →ₗ[ℂ] H) := by
      intro a ha
      exact K.orthogonal_mem_invtSubmodule_of_adjoint (a := a) <|
        by simpa [ContinuousLinearMap.star_eq_adjoint] using hK (star a) (star_mem ha)
    intro a ha
    simpa [eq_comm] using
      (Submodule.commute_starProjection_of_invtSubmodule (U := K) (a := a)
        (hK a ha) (hKorth ha)).eq

/-- The cyclic subspace generated by `ξ` under `S` is invariant under every element of `S`. -/
theorem cyclicSubmodule_mem_invtSubmodule
    (S : StarSubalgebra ℂ (H →L[ℂ] H)) (ξ : H) :
    ∀ {a : H →L[ℂ] H}, a ∈ S →
      S.toSubalgebra.toSubmodule.map (ContinuousLinearMap.apply ℂ H ξ).toLinearMap ∈
        Module.End.invtSubmodule (a : H →ₗ[ℂ] H) := by
  intro a ha
...
theorem double_centralizer_apply_mem_closure_image_apply
  ...
  have hK_invt : ∀ {a : H →L[ℂ] H}, a ∈ S → K ∈ Module.End.invtSubmodule (a : H →ₗ[ℂ] H) := by
    ...
    have hK0_invt : K0 ∈ Module.End.invtSubmodule (a : H →ₗ[ℂ] H) := by
      simpa [K0] using
        (cyclicSubmodule_mem_invtSubmodule (S := S) (ξ := ξ) (a := a) ha)
  ...
  have hp_mem : p ∈ Set.centralizer (S : Set (H →L[ℂ] H)) := by
    simpa [p] using
      (starProjection_mem_centralizer_iff_invtSubmodule (S := S) (K := K)).2
        (fun a ha => hK_invt ha)
  have hpx : p * x = x * p := hx p hp_mem
  have hxK_invt : K ∈ Module.End.invtSubmodule (x : H →ₗ[ℂ] H) := by
    have hpx_comm : Commute p x := by
      simpa [Commute] using hpx
    simpa [p] using
      (Submodule.invtSubmodule_of_commute_starProjection (U := K) (a := x) hpx_comm)
  have hxK : ∀ v ∈ K, x v ∈ K := (Module.End.mem_invtSubmodule (x : H →ₗ[ℂ] H)).1 hxK_invt
  have hξK0 : ξ ∈ K0 := by
    refine Submodule.mem_map.mpr ?_
